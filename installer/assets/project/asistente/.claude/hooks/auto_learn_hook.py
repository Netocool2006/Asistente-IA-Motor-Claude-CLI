...
# ── File Locking ──────────────────────────────────────────────
LOCK_DIR = Path.home() / ".adaptive_cli" / "locks"
...
HINT_EFFECT_FILE      = Path.home() / ".adaptive_cli" / "hint_effectiveness.json"
DEBUG_LOG             = Path.home() / ".adaptive_cli" / "hook_debug.log"
ITER_ACTIONS_FILE     = Path.home() / ".adaptive_cli" / "iteration_actions.jsonl"
...
def main():
    ...
    for fname in ("last_user_message.txt", "last_claude_action.txt"):
        try:
            p = Path.home() / ".adaptive_cli" / fname
            if p.exists():
                p.unlink()
        except Exception:
            pass
