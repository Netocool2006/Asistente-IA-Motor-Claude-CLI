...
ACTIONS_LOG       = Path.home() / ".adaptive_cli" / "iteration_actions.jsonl"  # append-only
NOTIFY_FILE       = Path.home() / ".adaptive_cli" / "last_learning.txt"
FINGERPRINTS_FILE = Path.home() / ".adaptive_cli" / "iter_fingerprints.json"
FAILURES_FILE     = Path.home() / ".adaptive_cli" / "pattern_failures.json"
...
def kb_save(actions, iteration_num):
    ...
    had_kb   = False
    try:
        _mtf = Path.home() / ".adaptive_cli" / "hook_state" / "msg_type.json"
        if _mtf.exists():
