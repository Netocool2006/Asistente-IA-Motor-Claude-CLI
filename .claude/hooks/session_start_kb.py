...
def recover_crashed_session() -> list:
    ...
    ACTIONS_LOG = DATA_DIR / "iteration_actions.jsonl"
...
def main():
    ...
    lines.append("")
    lines.append("  PROTOCOLO ANTI-COMPACTION (inspirado en Engram):")
    lines.append("  Si detectas que el contexto fue comprimido/compactado:")
    lines.append("  a) PRIMERO ejecutar: python learning_memory.py export")
    lines.append("     Esto recupera TODOS los patrones aprendidos.")
    lines.append("  b) SEGUNDO ejecutar: python knowledge_base.py cross-search --query \"ultimo trabajo\"")
    lines.append("     Esto recupera el contexto del KB.")
    lines.append("  c) TERCERO: leer ~/.adaptive_cli/session_history.json para ultimas sesiones.")
    lines.append("  d) Recien despues de recuperar contexto, continuar el trabajo.")
    lines.append("  Este protocolo GARANTIZA que no se pierde memoria tras compaction.")
    lines.append("=" * 60)
    ...
    # ═══════════════════════════════════════════════════════════
    try:
        last_msg_path = DATA_DIR / "last_user_message.txt"
